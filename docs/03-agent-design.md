...
**ingest.** Updates the profile from the user's message. Guided mode parses the pending question's answer deterministically (each discriminator owns its parser — works with zero LLM keys); free-form mode adds structured-output LLM extraction merged non-destructively. Fabricating values is the main failure mode — the prompt requires `null` over guesses. **Deterministic-parse-first (token economy):** a message that is *only* the direct answer to the pending question — an exact chip option, a bare number (`35`, `₱3,000`), or a short product-line pick (`Life`) — skips the extractor LLM entirely, since the deterministic parser already consumed everything; the PWA stays in freeform mode after a typed start, so chip taps would otherwise each pay for an extraction call. Any message carrying more than the direct answer (`35, and I smoke`, `3000 monthly`) still extracts, so no information is lost.
...
**explain.** Before calling the writer, checks the **explanation cache** (`app.explanation_cache`, `EXPLANATION_CACHE=auto`): the discriminator engine quantizes users into a bounded set of outcome buckets — the same profile answers against the same policy versions always yield the same candidate set — so writer + panel output is cached per bucket, keyed by a content hash of (writer model, judge panel, prompts, profile, verified policy content). A hit serves the already-verified result and skips the writer *and* the judge panel (cost goes O(users) → O(buckets)); a policy re-version or model/prompt change alters the hash, so invalidation is automatic. Cache errors read as a miss — the conversation never depends on it; rows unused past `EXPLANATION_CACHE_TTL_DAYS` (30) are purged by the retention task. On a miss, generates comparisons grounded ONLY in verified fields; every claim must reference an attribute. Without a writer (no provider key, or `LLM_ECONOMY=deterministic`), reasons are **template-rendered from verified fields** (`deterministic_reasons` in `verify.py`, copy in `prompts.py`): age within the eligible range, premium within budget, answered attributes stated back, and unstated asked-about attributes surfaced as `gap` — grounded by construction, zero tokens. Each reason is classified `match` (a criterion the user asked about is met, or a positive fact) or `gap` (a criterion the user asked about is missing from the policy data); a policy with any gap is a **partial** match, else **strong** (`match_strength`) — a structured signal the UI badges, not a phrasing guess. Includes disclaimers + "as of" freshness.

**verify_explanations (multi-LLM panel).** After `explain`, a panel of judge models — configured via `VERIFIER_MODELS`, at least two, ideally from different providers than the writer — independently fact-checks each positive `match` reason against the policy's verified fields. The panel is **batched** (token economy): each judge gets ONE call per policy carrying the policy facts once plus all numbered claims, returning one verdict per claim — instead of re-sending the same facts for every (judge, claim) pair, cutting panel calls and tokens from N×J to J per policy; a misaligned or failed response rejects every claim (fail closed). It fact-checks (honest `gap` notes about missing data aren't coverage claims, so they're never judged away — dropping them would make a partial match look strong). A match reason survives only on a unanimous "grounded" vote; failed reasons are dropped silently (a policy whose reasons all fail gets a generic fallback line — the panel never removes a policy, since `rank_and_verify` already validated it). A judge error counts as a rejection, and the whole node is a no-op when fewer than two judges are configured, so the panel can only ever make output stricter, never break it. After the panel, the final verified result is written to the explanation cache (see `explain`), so the next user in the same outcome bucket skips both LLM stages; on a cache hit the node is a no-op (the cached payload was verified when stored). The SSE `recommendations` event is emitted from THIS node, not from `explain` — streaming the writer's raw output would show the first user of an outcome bucket claims the panel then drops, while later (cached) users saw the filtered set.

**present.** Emits JSON the PWA renders natively (cards, compare matrix) — not a wall of markdown.

## Model configuration

Models are provider-agnostic `init_chat_model` strings, chosen from a 6-model roster of 3 tiers (2 each; `agent/config.py` resolves each tier's `_1` slot, with legacy `LLM_MODEL`/`LLM_MODEL_SMALL` as the large/small fallback). This package uses writer = **large** (`LLM_MODEL_LARGE_1`), extractor = **small** (`LLM_MODEL_SMALL_1`); judges = `VERIFIER_MODELS` (≥2, cross-provider). The **mid** tier drives ingestion (vision/intake/correction). The `LLM_ECONOMY` ladder (`agent/economy.py`) is the spend kill switch: `full` (default) runs everything; `lean` drops the judge panel (safe — the panel only ever makes output stricter); `deterministic` makes zero LLM calls (template explanations + deterministic parsing — the same posture as running keyless). The mode participates in the explanation-cache key so leaner results never serve as fully-verified ones. Every LLM call is metered into the `app.llm_usage` day-ledger via LangChain usage callbacks — (day, model, role) → calls + input/output tokens, with explanation-cache hits recorded as zero-token rows so avoided spend stays visible; `GET /ops/usage` exposes the ledger and `DAILY_TOKEN_BUDGET` logs a once-per-day WARNING when crossed (observability only — pair it with an `LLM_ECONOMY` downshift). LangSmith remains the per-call tracing tool; the ledger is the always-on spend counter that needs no SaaS. Two provider-specific details are load-bearing:
