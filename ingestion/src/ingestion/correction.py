...
When a reviewer approves a draft that fails validation, the mid-tier model
re-reads the document images WITH the exact error and returns a corrected draft
for another human review — capped at MAX_CORRECTION_ATTEMPTS. Reuses the vision
page renderer (pypdfium2) and the same ``policy_draft_schema`` as extraction, so
the corrected draft is contract-identical. Uses the mid tier (``LLM_MODEL_MID_1``),
...
from ingestion import models, vision
...
async def correct_draft(data: bytes, draft: dict, errors: str) -> dict | None:
    ...
    the mid-tier model and return a corrected draft dict. Returns None on any
    ...
    try:
        ...
        model = init_chat_model(models.mid()).with_structured_output(
            policy_draft_schema()
        )
