...
**Everything reports the same number.** Each service returns it from `/health`, and the PWA shows it in the footer and serves it at `/VERSION`. Both read `APP_VERSION`, which `docker-compose.prod.yml` sets to `$IMAGE_TAG` — necessary because selective rebuilds re-tag an unchanged image into later releases, so the number baked in at build time goes stale while the deployment moves on. The PWA gets it via `config.js`, rendered per-container and never precached. At runtime the version resolves as `APP_VERSION` env → the baked `VERSION` file → `0.0.0+dev`, and anything that is **not** a published artifact gets a `+dev` suffix (detected by the absence of the `BUILD_ID` CI writes for every image it publishes). So a source checkout reports `0.1.1+dev` — the floor, plainly marked — rather than claiming to be release 0.1.1. `docker-compose.prod.yml` passes `APP_VERSION=$IMAGE_TAG`, so a version-pinned deploy reports that release across every service.
