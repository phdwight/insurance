...
- **Policy data intake:** manual/document upload (public brochures, PDFs with tables) → parse (PDFs: the frontier vision `LLM_MODEL` triages each upload — it transcribes image-heavy/scanned docs to markdown itself, else routes to docling; `VISION_TRIAGE` toggles it, failures degrade to docling) → LLM extraction → **mandatory human review** → publish. No insurer APIs or scraping for MVP. Tables are extracted into structured JSONB fields and queried with SQL — they never reach the embedder.
